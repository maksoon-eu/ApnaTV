...
const LickedSlider = () => {
    const { licked } = useContext(LickedContext);

    const lickedFilms = licked.map(item => {

        return (
            <LickedSliderItem item={item} key={item.id}/>
        )
    });
    ...
    return (
            <motion.div
                initial={{ opacity: 0 }}
                variants={variants}
                animate={licked.length > 0 ? "open" : 'close'}
                className="choseFilm__slider"
            >
                <AnimatePresence>
                {licked.length > 0 ? <div>
